...
if ($current === 'active') {
    respond(true, 'Project resumed', [
        'id' => $projectId,
        'status' => 'active',
    ]);
}
if ($current !== 'pending') {
    respond(false, 'Project is not pending', null, 422);
}

$conn->begin_transaction();
try {
    $stmt = $conn->prepare("UPDATE projects SET status = 'active' WHERE id = ? AND contractor_id = ? AND status = 'pending'");
    $stmt->bind_param('ii', $projectId, $contractorId);
    if (!$stmt->execute()) {
        throw new RuntimeException('Failed to resume project');
    }
    $stmt->close();
